	...
	return (
		<KanbanBoardContext.Provider value={true}>
			<SortableContext
				items={columns}
				strategy={
					context.orientation === "horizontal" ? horizontalListSortingStrategy : verticalListSortingStrategy
				}
			>
				<BoardPrimitive
...
function KanbanColumn(props: KanbanColumnProps) {
	...
	const composedRef = useComposedRefs(ref, node => {
		if (disabled) return;
		setNodeRef(node as HTMLElement); // Explicitly type node
	});
...
function KanbanColumnHandle(props: KanbanColumnHandleProps) {
	...
	const composedRef = useComposedRefs(ref, node => {
		...
		columnContext.setActivatorNodeRef(node as HTMLElement); // Explicitly type node
	});
...
function KanbanItemHandle(props: KanbanItemHandleProps) {
	...
	const composedRef = useComposedRefs(ref, node => {
		if (isDisabled) return;
		itemContext.setActivatorNodeRef(node as HTMLElement); // Explicitly type node
	});
