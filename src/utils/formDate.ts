export type MeasurementType = "Weight" | "Height" | "BMI" | "HeadCircumference";
...
export const getAvatarColor = (doctorId?: string) => {
	...
	const hash = doctorId.split("").reduce((acc, char) => Math.imul(31, acc) + char.charCodeAt(0), 0);
...
export function getYAxisLabel(type: MeasurementType): string {
	switch (type) {
		case "Weight":
			return "Weight (kg)";
		case "Height":
...
export function getPercentileVariant(percentile: number): "default" | "secondary" | "destructive" | "outline" {
