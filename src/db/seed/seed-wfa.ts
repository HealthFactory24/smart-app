...
function calculateAgeInMonths(ageDays: number): number {
	...
	return Number.parseFloat((ageDays / 30.4375).toFixed(2)); // Keep 2 decimal places for accuracy
}

function mapDataPoint(dataPoint: ZScoreDataPoint, gender: Gender): ValidatedGrowthRecord {
	...
	return {
		...
		measurementType: "WEIGHT", // WFA maps to WEIGHT in the schema enum
...
export default async function wfaSeed(db: AppDb) {
	console.log("--- Starting WHO Growth Standards Seeding ---");

	let whoJson: ZScoreData;
	try {
		whoJson = JSON.parse(fs.readFileSync(ZSCORE_DATA_PATH, "utf-8")) as ZScoreData;
		if (!whoJson?.wfa) {
	...
	if (whoJson.wfa.boys && Array.isArray(whoJson.wfa.boys)) {
		...
		for (const dataPoint of whoJson.wfa.boys) {
			try {
				const mappedRecord = mapDataPoint(dataPoint, Gender.MALE);
	...
	if (whoJson.wfa.girls && Array.isArray(whoJson.wfa.girls)) {
		...
		for (const dataPoint of whoJson.wfa.girls) {
			try {
				const mappedRecord = mapDataPoint(dataPoint, Gender.FEMALE);
				allRecords.push(mappedRecord);
	...
	for (const days of sampleAges) {
		...
		console.log(`  ${days} days = ${months.toFixed(2)} months`);
	}
	...
	if (dataToInsert.length > 0) {
		try {
			...
			for (let i = 0; i < dataToInsert.length; i += batchSize) {
				...
				console.log(`  ✅ Inserted ${batch.length} records in batch ${batchNumber}.`);
			}
